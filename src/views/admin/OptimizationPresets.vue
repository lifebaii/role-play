  ...
  <div class="p-4 sm:p-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-xl bg-gradient-to-r from-[var(--theme-accent)] to-[var(--theme-primary)] flex items-center justify-center shadow-lg shadow-[var(--theme-accent)]/25">
          <svg class="w-5 h-5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"></path>
          </svg>
        </div>
        <div>
          <h1 class="text-xl sm:text-2xl font-bold text-theme-text-primary">优化预设</h1>
          <p class="text-sm text-theme-text-secondary">管理角色数据优化预设</p>
        </div>
      </div>
    ...
    <div class="space-y-4">
      <div v-if="presets.length === 0" class="text-center py-16">
        ...
        <p class="text-theme-text-primary font-medium mb-1">暂无优化预设</p>
        ...
        <template #item="{ element, index }">
          <div class="chat-card rounded-xl border border-theme-border shadow-sm hover:shadow-md transition-all overflow-hidden">
            ...
            <div class="p-4 space-y-4">
              <div>
                <label class="block text-xs font-medium text-theme-text-secondary mb-2">使用模型</label>
                <select
                  v-model="element.model_id"
              ...
              <div>
                <label class="block text-xs font-medium text-theme-text-secondary mb-2">系统提示词</label>
                <textarea
                  v-model="element.system_prompt"
                  rows="6"
                  class="w-full px-4 py-3 chat-input-field border border-theme-border rounded-xl text-theme-text-primary text-sm placeholder-theme-text-secondary/60 focus:ring-2 focus:ring-[var(--theme-primary)] focus:border-transparent transition-all resize-y min-h-[180px] font-mono"
                  placeholder="输入系统提示词..."
                ></textarea>
                <div class="flex items-center justify-between mt-2">
                  <span class="text-xs text-theme-text-secondary">{{ (element.system_prompt || '').length }} 字符</span>
                </div>
              </div>
            </div>
          </div>
...
import { optimizationPresetsApi, modelsApi } from '@/api'
...
async function loadPresets() {
  try {
    presets.value = await optimizationPresetsApi.list()
  } catch (e) {
...
async function savePresets() {
  try {
    isSaving.value = true
    await optimizationPresetsApi.update(presets.value)
...
async function handleImport(event: Event) {
  ...
  try {
    ...
    // 验证导入的数据格式
    const validData = data.filter((item: any) => item.name && item.system_prompt)
    if (validData.length !== data.length) {
      await showAlert(`导入了 ${validData.length} 个有效预设（跳过了 ${data.length - validData.length} 个无效项）`)
    } else {
      await showAlert(`成功导入 ${validData.length} 个预设`)
    }

    presets.value = [...presets.value, ...validData]
  } catch (err: any) {
